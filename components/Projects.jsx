...
export default function Projects() {
    ...
    useEffect(() => {
        promise(setProjects(projectsData));
    }, [id])
        ...
        <main id="projects" className='w-full md:h-full py-24 items-center justify-center'>
            <div className='max-w-[1280px] h-full mx-auto'>
                ...
                <section className='grid p-4 grid-cols-1 gap-8 md:grid-cols-2'>
                    {projects.map(p => {
                        return (<div key="id" className='flex flex-col h-auto w-full p-8 items-center justify-center shadow-lg shadow-blue-300 rounded-xl relative cursor-pointer hover:shadow-xl hover:shadow-blue-400 ease-in-out duration-300'>
                            <Image className="rounded-xl" src={p.thumbnail} alt="/" width='400' height='200' />
                            <div>
                                <div className="pb-4">
                                    <h6 className="mt-4 py-2 text-center border-t-2 border-gray-200">{p.name}</h6>
                                    <p className="font-thin text-sm text-center text-gray-500">{p.responsive}</p>
                                </div>
                            ...
                            <div className="flex mt-4 justify-center items-center gap-4">
                                <span className="p-2 rounded-full shadow-sm shadow-gray-300 cursor-pointer hover:shadow-md hover:shadow-gray-400 ease-in-out duration-200">
                                    <a href={p.repo} target='_blank' rel="noreferrer"><FaGithub /></a>
                                </span>
                                <span className="p-2 rounded-full shadow-sm shadow-gray-300 cursor-pointer hover:shadow-md hover:shadow-gray-400 ease-in-out duration-200">
                                    <a href={p.deploy} target='_blank' rel="noreferrer"><IoMdOpen /></a>
                                </span>
                            </div>
                        </div>)
                    })}
